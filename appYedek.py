...
def parse_gemma_sections(text):
    ...
    parsed = {key: "" for key in ["Kıyafet Analizi", "Renk Uyumu", "Kombin Önerisi", "Kombin Yorumu", "Alternatif Kombin"]}
...
def kombin_olustur():
    ...
    if len(files) < 2:
        return jsonify({"error": "En az 2 resim yükleyin."}), 400
...
Lütfen aşağıdaki başlıklara kısa (1-2 cümle) cevaplar ver:
Kıyafet Analizi:
Renk Uyumu:
Kombin Önerisi:
Kombin Yorumu:
...
"""
    ...
    yanit = gemma_generate(prompt)
    parsed = parse_gemma_sections(yanit)
...
Kıyafet Yorumu:
"""
    yanit = gemma_generate(full_prompt, predict=150)
    try:
        temiz = yanit.split("Kıyafet Yorumu:")[1].strip()
    except:
        temiz = yanit.strip()

    return jsonify({"yorum": temiz})
...
Lütfen kısa ve öz bir kıyafet yorumu yap, kıyafete eklenebilecek ve giyilebleceği yere göre değerlendir ve yorumu ona göre yap, kombini şık gösterecek detayları ekle ve sadece "Kıyafet Yorumu:" kısmını oluştur.
Kıyafet Yorumu:
"""
